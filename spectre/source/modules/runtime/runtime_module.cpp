...
namespace spectre::modules {
    ...
    static void register_scripts_from_directory(flecs::world& world, const char* virtual_path) {
        if (!sandbox::modules::filesystem::exists(world, virtual_path)) return;
    ...
    bool runtime_module_t::initialize(flecs::world& entity_world) {
        ...
        if (sandbox::modules::filesystem::exists(entity_world, "app://resources/configs/window.json")) {
            std::string content = sandbox::modules::filesystem::read_all_text(entity_world, "app://resources/configs/window.json");
            sandbox::properties props(content, sandbox::properties::Format::JSON);
            ...
            sandbox::modules::logs::warn(entity_world, "[Runtime Module] Window configuration missing at app://resources/configs/window.json");
        }

        // Renderer Registration
        if (sandbox::modules::filesystem::exists(entity_world, "app://resources/configs/renderer.json")) {
            std::string content = sandbox::modules::filesystem::read_all_text(entity_world, "app://resources/configs/renderer.json");
            sandbox::properties props(content, sandbox::properties::Format::JSON);
            ...
            sandbox::modules::logs::warn(entity_world, "[Runtime Module] Renderer configuration missing at app://resources/configs/renderer.json");
        }

        // Default State Stack
        if (sandbox::modules::filesystem::exists(entity_world, "app://resources/configs/startup.json")) {
            std::string content = sandbox::modules::filesystem::read_all_text(entity_world, "app://resources/configs/startup.json");
            sandbox::properties props(content, sandbox::properties::Format::JSON);
            ...
            if (props.get_array<std::string>("states", startup_states)) {
                ...
                sandbox::modules::logs::trace(entity_world, "[Runtime Module] Initialized state stack with {} states.", startup_states.size());
            }
        }
