...
void runtime_module_t::main_loop(flecs::world &entity_world) {
  sandbox::modules::logs::info(entity_world,
                               "[Runtime Module] Runtime loop started");

  while (m_running->load() && entity_world.progress()) {
    if (m_paused->load()) {
      std::unique_lock<std::mutex> lock(*m_mutex);
      sandbox::modules::logs::info(entity_world,
                                   "[Runtime Module] Runtime paused");
      m_cv->wait(lock,
                 [this]() { return !m_paused->load() || !m_running->load(); });
      sandbox::modules::logs::info(entity_world,
                                   "[Runtime Module] Runtime resumed");
    }
  }

  sandbox::modules::logs::info(entity_world,
                               "[Runtime Module] Runtime loop stopped");
}

static void register_scripts_from_directory(flecs::world &world,
                                            const char *virtual_path) {
  if (!sandbox::modules::filesystem::exists(world, virtual_path))
    return;
  auto files =
      sandbox::modules::filesystem::list_files(world, virtual_path, true);
  for (const auto &file : files) {
    sandbox::modules::logs::trace(
        world, "[Runtime Module] Registering script: {}", file);
    spectre::modules::scripts::include_code(world, file);
  }
}

static void register_prefabs_from_directory(flecs::world &world,
                                            const char *virtual_path) {
  if (!sandbox::modules::filesystem::exists(world, virtual_path))
    return;
  auto files =
  ...
  for (const auto &file : files) {
    sandbox::modules::logs::trace(
        world, "[Runtime Module] Registering prefab: {}", file);
    std::string content =
        sandbox::modules::filesystem::read_all_text(world, file.c_str());
    if (!content.empty()) {
      ...
      spectre::modules::prefabs::register_prefab(world, prefab_name.c_str(),
                                                 props.get_raw());
    }
  }
}

static void register_scenes_from_directory(flecs::world &world,
                                           const char *virtual_path) {
  if (!sandbox::modules::filesystem::exists(world, virtual_path))
    return;
  auto files =
  ...
  for (const auto &file : files) {
    sandbox::modules::logs::trace(
        world, "[Runtime Module] Registering scene: {}", file);
    std::string content =
        sandbox::modules::filesystem::read_all_text(world, file.c_str());
    if (!content.empty()) {
      sandbox::properties props(content, sandbox::properties::Format::JSON);
...
static void register_states_from_directory(flecs::world &world,
                                           const char *virtual_path) {
  if (!sandbox::modules::filesystem::exists(world, virtual_path))
    return;
  auto files =
  ...
  for (const auto &file : files) {
    sandbox::modules::logs::trace(
        world, "[Runtime Module] Registering state: {}", file);
    std::string content =
        sandbox::modules::filesystem::read_all_text(world, file.c_str());
    if (!content.empty()) {
      sandbox::properties props(content, sandbox::properties::Format::JSON);
...
bool runtime_module_t::initialize(flecs::world &entity_world) {
  sandbox::modules::logs::info(
      entity_world, "[Runtime Module] Initializing runtime resources...");

  register_scripts_from_directory(entity_world,
                                  "app://resources/assets/scripts");
  register_prefabs_from_directory(entity_world,
                                  "app://resources/scenes/prefabs");
  ...
  // Window Registration
  if (sandbox::modules::filesystem::exists(
          entity_world, "app://resources/configs/window.json")) {
    ...
    sandbox::modules::logs::trace(entity_world,
                                  "[Runtime Module] Registered window config.");
  } else {
    sandbox::modules::logs::warn(
        entity_world, "[Runtime Module] Window configuration missing at "
                      "app://resources/configs/window.json");
  }

  // Renderer Registration
  if (sandbox::modules::filesystem::exists(
          entity_world, "app://resources/configs/renderer.json")) {
    std::string content = sandbox::modules::filesystem::read_all_text(
        entity_world, "app://resources/configs/renderer.json");
    sandbox::properties props(content, sandbox::properties::Format::JSON);
    spectre::modules::renderer::register_renderer(entity_world,
                                                  props.get_raw());
    sandbox::modules::logs::trace(
        entity_world, "[Runtime Module] Registered renderer config.");
  } else {
    sandbox::modules::logs::warn(
        entity_world, "[Runtime Module] Renderer configuration missing at "
                      "app://resources/configs/renderer.json");
  }

  // Default State Stack
  if (sandbox::modules::filesystem::exists(
          entity_world, "app://resources/configs/startup.json")) {
    std::string content = sandbox::modules::filesystem::read_all_text(
        entity_world, "app://resources/configs/startup.json");
    sandbox::properties props(content, sandbox::properties::Format::JSON);
    ...
    if (props.get_array<std::string>("states", startup_states)) {
      for (const auto &state_name : startup_states) {
        ecs_entity_t state_id = spectre::modules::scenes::find_state(
            entity_world, state_name.c_str());
        if (state_id) {
      ...
      sandbox::modules::logs::trace(
          entity_world,
          "[Runtime Module] Initialized state stack with {} states.",
          startup_states.size());
    }
  }

  sandbox::modules::logs::info(
      entity_world, "[Runtime Module] Runtime initialization complete.");
  return true;
}
...
void runtime_module_t::run(flecs::world &entity_world) {
  if (m_thread) {
    sandbox::modules::logs::warn(
        entity_world,
        "[Runtime Module] Runtime is already running in a thread.");
    return;
  }
...
void runtime_module_t::start(flecs::world &entity_world) {
  if (m_thread) {
    sandbox::modules::logs::warn(
        entity_world, "[Runtime Module] Runtime is already running.");
    return;
  }

  initialize(entity_world);
  m_running->store(true);
  m_thread = std::make_unique<std::thread>(&runtime_module_t::main_loop, this,
                                           std::ref(entity_world));
}
...
void runtime_module_t::pause() { m_paused->store(true); }
...
static sandbox_runtime_api_t g_spectre_runtime_api = {
    .run = spectre_runtime_run,
...
struct runtime_module_initializer_t {
  runtime_module_initializer_t(flecs::world &entity_world) {
    ...
    sandbox_runtime_service_t *service =
        entity_world.try_get_mut<sandbox_runtime_service_t>();
    if (service) {
