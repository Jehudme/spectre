...
static void
deserialize_entity_cb(ecs_world_t *world, ecs_entity_t entity,
                      sandbox_properties_handle_t properties_handle);
static sandbox_properties_handle_t serialize_entity_cb(ecs_world_t *world,
...
prefabs_module_t::prefabs_module_t(flecs::world &world) : m_world(world) {
  ...
  spectre::modules::serializer::register_serializer(m_world, "entity",
                                                    &entity_serializer);
  ...
  m_world.observer<spectre_use_script_on_destroy_relation_t>()
      ...
      .each([](flecs::iter &iterator, size_t index,
               spectre_use_script_on_destroy_relation_t &relation) {
        flecs::entity entity = iterator.entity(index);
        flecs::entity target = entity.parent();
        if (!target.is_valid() || target.name() != "scripts")
          target = entity;
        else
  ...
  sandbox::modules::logs::info(m_world,
                               "[Prefabs Module] Initialized successfully.");
}
...
deserialize_entity_cb(ecs_world_t *world, ecs_entity_t entity,
                      sandbox_properties_handle_t properties_handle) {
  spectre::modules::prefabs::deserialize_entity(flecs::world(world), entity,
                                                properties_handle);
}

static sandbox_properties_handle_t serialize_entity_cb(ecs_world_t *world,
                                                       ecs_entity_t entity_id) {
  return spectre::modules::prefabs::serialize_entity(flecs::world(world),
                                                     entity_id);
}

sandbox::properties
prefabs_module_t::serialize_entity(flecs::entity entity_to_serialize) {
  sandbox::properties result_properties;
  if (!entity_to_serialize.is_valid()) {
    sandbox::modules::logs::warn(
        m_world, "[Prefabs Module] Cannot serialize invalid entity.");
    return result_properties;
  }

  if (entity_to_serialize.name().c_str()) {
    result_properties.set<std::string>("name",
                                       entity_to_serialize.name().c_str());
  }
  ...
  entity_to_serialize.each([&](flecs::id id) {
    ...
    if (!comp_entity.is_valid() || !comp_entity.name().c_str())
      return;

    std::string s_name = comp_entity.name().c_str();
    ecs_entity_t serializer_id =
        spectre::modules::serializer::find_serializer(m_world, s_name.c_str());
    if (serializer_id != 0) {
      sandbox_properties_handle_t props_handle =
          spectre::modules::serializer::serialize_entity(
              m_world, serializer_id, entity_to_serialize.id());
  ...
  if (m_script_args_serializer.id() != 0) {
    sandbox_properties_handle_t scripts_handle =
        spectre::modules::serializer::serialize_entity(
            m_world, m_script_args_serializer.id(), entity_to_serialize.id());
  ...
  entity_to_serialize.children([&](flecs::entity child) {
    ...
    if (child_properties.is_valid()) {
      std::string child_key = child.name().c_str() ? child.name().c_str()
...
flecs::entity
prefabs_module_t::deserialize_entity(flecs::entity target_entity,
  ...
  if (properties.has("components")) {
    ...
    for (const auto &component_name : component_keys) {
      sandbox::properties component_properties =
          components_node.sub(component_name);
      if (component_properties.is_valid()) {
        ecs_entity_t serializer_id =
            spectre::modules::serializer::find_serializer(
                m_world, component_name.c_str());
        if (serializer_id != 0) {
          spectre::modules::serializer::deserialize_entity(
              m_world, serializer_id, target_entity.id(),
              component_properties.get_raw());
        } else {
          sandbox::modules::logs::info(m_world,
                                       "[Prefabs Module] No serializer for "
                                       "component '{}', trying fallback lookup",
                                       component_name);
    ...
    for (const auto &child_key : child_keys) {
      ...
      if (child_properties.is_valid()) {
        ...
        flecs::entity child_entity =
            deserialize_entity(new_child, std::move(child_properties));
...
void prefabs_module_t::register_prefab(std::string_view name,
                                       sandbox::properties properties) {
  if (name.empty()) {
    sandbox::modules::logs::error(
        m_world, "[Prefabs Module] Cannot register prefab with empty name.");
    return;
  }
  ...
  if (properties.is_valid()) {
    ...
    sandbox::modules::logs::info(
        m_world, "[Prefabs Module] Registered prefab: '{}'", name);
  }
}
...
bool prefabs_module_t::is_prefab(flecs::entity entity_to_check) const {
  return entity_to_check.is_valid() &&
         entity_to_check.has(flecs::IsA, m_entity_prefab);
}
...
static void run_on_create_scripts(flecs::entity target_entity,
                                  flecs::world &world) {
  ...
  target_entity.children(
      [&](flecs::entity child) { run_on_create_scripts(child, world); });
...
flecs::entity prefabs_module_t::create_entity(sandbox::properties properties) {
  ...
  flecs::entity created_entity =
      deserialize_entity(new_entity, std::move(properties));
