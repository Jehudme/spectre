...
sandbox::properties prefabs_module_t::serialize_entity(flecs::entity entity_to_serialize) {
    ...
    entity_to_serialize.each([&](flecs::id id) {
        ...
        if (serializer_id != 0) {
            sandbox_properties_handle_t props_handle = spectre::modules::serializer::serialize_entity(
    ...
    if (m_script_args_serializer.id() != 0) {
        sandbox_properties_handle_t scripts_handle = spectre::modules::serializer::serialize_entity(
    ...
    if (properties.has("components")) {
        ...
        for (const auto& component_name : component_keys) {
            ...
            if (component_properties.is_valid()) {
                ecs_entity_t serializer_id = spectre::modules::serializer::find_serializer(m_world,
                                                                                           component_name.c_str());
                if (serializer_id != 0) {
                    spectre::modules::serializer::deserialize_entity(m_world, serializer_id, target_entity.id(),
...
void prefabs_module_t::register_prefab(std::string_view name, sandbox::properties properties) {
    ...
    if (!prefab.is_valid()) {
        prefab = m_world.prefab(std::string(name).c_str()).child_of(m_prefabs_root).is_a(m_entity_prefab);
    }
...
static void run_on_create_scripts(flecs::entity target_entity, flecs::world& world) {
    ...
    target_entity.children([&](flecs::entity child) { run_on_create_scripts(child, world); });
