...
static int custom_vfs_loader(lua_State *L) {
  ...
  auto *world =
      static_cast<ecs_world_t *>(lua_touserdata(L, lua_upvalueindex(1)));

  std::string path1 = std::string("app://resources/assets/scripts/externals/") +
                      modname + ".lua";
  std::string path2 =
      std::string("app://resources/assets/scripts/") + modname + ".lua";

  uint8_t *data = nullptr;
  size_t data_size = 0;

  if (!sandbox_filesystem_read_all_bytes(world, path1.c_str(), &data,
                                         &data_size)) {
    if (!sandbox_filesystem_read_all_bytes(world, path2.c_str(), &data,
                                           &data_size)) {
      ...
      std::string path3 =
          std::string("app://resources/assets/scripts/") + modpath + ".lua";
      if (!sandbox_filesystem_read_all_bytes(world, path3.c_str(), &data,
                                             &data_size)) {
...
script_module_t::script_module_t(flecs::world &world)
    : m_world(world), m_ffi_initialized(false) {
  sandbox::modules::logs::trace(const_cast<flecs::world &>(m_world),
                                "[Scripts Module] Initializing...");
  ...
  auto deserialize_empty = [](ecs_world_t *, ecs_entity_t,
                              sandbox_properties_handle_t) {};
  auto serialize_empty = [](ecs_world_t *,
                            ecs_entity_t) -> sandbox_properties_handle_t {
    return {0};
  };
  spectre_serializer_component empty_serializer = {deserialize_empty,
                                                   serialize_empty};

  register_script_comp(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(m_world, "spectre_script_t",
                                                    &empty_serializer);

  register_use_script_on_create_rel(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_use_script_on_create_relation_t", &empty_serializer);

  register_use_script_on_destroy_rel(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_use_script_on_destroy_relation_t", &empty_serializer);

  register_use_script_on_update_rel(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_use_script_on_update_relation_t", &empty_serializer);

  register_use_script_on_enter_rel(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_use_script_on_enter_relation_t", &empty_serializer);

  register_use_script_on_exit_rel(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_use_script_on_exit_relation_t", &empty_serializer);
  ...
  spectre::modules::serializer::register_serializer(m_world, "scripts",
                                                    &script_serializer);

  sandbox::modules::logs::info(const_cast<flecs::world &>(m_world),
                               "[Scripts Module] Initialized successfully.");
}
...
deserialize_script_args_cb(ecs_world_t *world, ecs_entity_t entity,
  ...
  if (module)
    module->deserialize_scripts(flecs::world(world).entity(entity),
                                sandbox::properties(properties_handle, false));
}

void script_module_t::deserialize_scripts(flecs::entity target_entity,
                                          sandbox::properties properties) {
  if (!properties.is_valid() || !target_entity.is_valid())
    return;

  auto deserialize_relation = [&](const char *relation_name,
                                  auto relation_type) {
    ...
    for (const auto &index_str : item_indices) {
      ...
      flecs::entity script_entity =
          m_scripts_root.lookup(function_name.c_str());
      ...
      const spectre_script_t *script_component =
          script_entity.try_get<spectre_script_t>();
      if (!script_component)
        continue;
      ...
      if (argument_count > 0) {
        ...
        for (size_t index = 0; index < argument_count; ++index) {
          ...
          spectre_script_argument_type_t argument_type =
              script_component->argument_types[index];
          ...
          case SPECTRE_SCRIPT_ARGUMENT_TYPE_STRING: {
            ...
            script_arguments[index].value.string_value =
                strdup(string_value.c_str());
            break;
          }
  ...
  deserialize_relation("on_destroy",
...
sandbox::properties
script_module_t::serialize_scripts(flecs::entity entity_to_serialize) {
  ...
  auto serialize_relation = [&](const char *relation_name, auto relation_type) {
    ...
    entity_to_serialize.each<RelationType>([&](flecs::entity script_entity) {
      const RelationType *relation_data_pointer =
          entity_to_serialize.try_get<RelationType>(script_entity);
      if (!relation_data_pointer)
        return;
      const RelationType &relation_data = *relation_data_pointer;

      sandbox::properties item_properties;
      item_properties.set<std::string>("function",
                                       script_entity.name().c_str());

      sandbox::properties arguments_properties;
      const spectre_script_t *script_component =
          script_entity.try_get<spectre_script_t>();
      if (script_component && relation_data.arguments &&
          relation_data.argument_count > 0) {
        for (size_t index = 0; index < relation_data.argument_count &&
                               index < script_component->argument_count;
             ++index) {
          const char *argument_name = script_component->arguments_name[index];
          spectre_script_argument_type_t argument_type =
              script_component->argument_types[index];
  ...
  while (std::getline(stream, line)) {
    ...
    if (std::regex_search(line, param_match, param_regex)) {
      ...
      if (type_string == "nil")
        ...
        argument_type = SPECTRE_SCRIPT_ARGUMENT_TYPE_BOOLEAN;
      else if (type_string == "number")
        argument_type = SPECTRE_SCRIPT_ARGUMENT_TYPE_NUMBER;
      else if (type_string == "integer")
        argument_type = SPECTRE_SCRIPT_ARGUMENT_TYPE_INTEGER;
      else if (type_string == "string")
        argument_type = SPECTRE_SCRIPT_ARGUMENT_TYPE_STRING;
      else if (type_string == "table")
        argument_type = SPECTRE_SCRIPT_ARGUMENT_TYPE_TABLE;
      else if (type_string == "userdata")
        argument_type = SPECTRE_SCRIPT_ARGUMENT_TYPE_USERDATA;
      else if (type_string == "entity")
        argument_type = SPECTRE_SCRIPT_ARGUMENT_TYPE_ENTITY;
      else {
        sandbox::modules::logs::warn(
            const_cast<flecs::world &>(m_world),
            "[Scripts Module] Unknown parameter type '{}' in Lua script.",
            type_string);
        argument_type = SPECTRE_SCRIPT_ARGUMENT_TYPE_NIL;
      }
    ...
    if (std::regex_search(line, function_match, function_regex)) {
      ...
      auto arguments_begin = std::sregex_iterator(
          arguments_string.begin(), arguments_string.end(), argument_regex);
      auto arguments_end = std::sregex_iterator();
      for (std::sregex_iterator iterator = arguments_begin;
           iterator != arguments_end; ++iterator) {
      ...
      if (pending_argument_types.size() != argument_count) {
        sandbox::modules::logs::error(
            const_cast<flecs::world &>(m_world),
            "[Scripts Module] Parser Error: Function '{}' expects {} "
            "parameters based on arguments, but {} @param types were provided.",
            function_name, argument_count, pending_argument_types.size());
      ...
      script.arguments_name =
          argument_count > 0 ? m_ptr_pool.back().data() : nullptr;
      script.argument_types =
          argument_count > 0 ? m_type_pool.back().data() : nullptr;
...
bool script_module_t::has_script(
    std::string_view function_name,
    const std::vector<script_argument_type_t> &arguments) const {
  if (function_name.empty())
    return false;
  flecs::entity script_entity =
      m_scripts_root.lookup(std::string(function_name).c_str());
  if (!is_script(script_entity))
    return false;
...
bool script_module_t::is_script(flecs::entity entity_to_check) const {
  return entity_to_check.is_valid() &&
         entity_to_check.has(flecs::IsA, m_script_prefab);
}
...
void script_module_t::include_code(std::string_view file_path) {
  std::string source_code = sandbox::modules::filesystem::read_all_text(
      const_cast<flecs::world &>(m_world), file_path.data());
  if (source_code.empty()) {
    sandbox::modules::logs::error(
        const_cast<flecs::world &>(m_world),
        "[Scripts Module] Failed to open/read script: {}", file_path);
    return;
  }
  ...
  if (luaL_dostring(m_lua, source_code.c_str()) != LUA_OK) {
    ...
    sandbox::modules::logs::error(const_cast<flecs::world &>(m_world),
                                  "[Scripts Module] Lua Error: {}",
                                  error_message);
    lua_pop(m_lua, 1);
  } else {
    sandbox::modules::logs::info(
        const_cast<flecs::world &>(m_world),
        "[Scripts Module] Successfully included script: {}", file_path);
  }
}

void script_module_t::execute_script(std::string_view function_name,
                                     script_arguments_t &arguments) {
  if (!has_script(function_name, {})) {
    sandbox::modules::logs::error(
        const_cast<flecs::world &>(m_world),
        "[Scripts Module] Cannot execute unknown script: {}", function_name);
    return;
  }
  ...
  if (script_component->argument_count != arguments.size()) {
    sandbox::modules::logs::error(
        ...
        "provided.",
        function_name, script_component->argument_count, arguments.size());
    return;
  }
  ...
  for (size_t index = 0; index < arguments.size(); ++index) {
    if (arguments[index].type != script_component->argument_types[index]) {
      sandbox::modules::logs::error(
          ...
          "{}, got {}.",
          function_name, index, (int)script_component->argument_types[index],
          (int)arguments[index].type);
      return;
    }
  }
  ...
  if (!lua_isfunction(m_lua, -1)) {
    ...
    sandbox::modules::logs::error(
        const_cast<flecs::world &>(m_world),
        "[Scripts Module] Lua global '{}' is not a function.", function_name);
    return;
  }
  ...
  if (lua_pcall(m_lua, arguments.size(), 0, 0) != LUA_OK) {
    const char *error_message = lua_tostring(m_lua, -1);
    sandbox::modules::logs::error(const_cast<flecs::world &>(m_world),
                                  "[Scripts Module] Error executing '{}': {}",
...
namespace {
flecs::entity resolve_entity_argument(flecs::entity target_entity,
...
void script_module_t::execute_script_with_target(
    flecs::entity target_entity, flecs::entity script_entity,
  ...
  if (script_component->argument_count != argument_count) {
    sandbox::modules::logs::error(
        ...
        "[Scripts Module] Script '{}' expects {} arguments, but {} were "
        "provided.",
        function_name, script_component->argument_count, argument_count);
    return;
  }

  for (size_t index = 0; index < argument_count; ++index) {
    if (script_arguments[index].type !=
        script_component->argument_types[index]) {
      sandbox::modules::logs::error(
          const_cast<flecs::world &>(m_world),
          "[Scripts Module] Script '{}' argument {} type mismatch. Expected "
  ...
  for (size_t index = 0; index < argument_count; ++index) {
    ...
    case SPECTRE_SCRIPT_ARGUMENT_TYPE_USERDATA:
      ...
      break;
    case SPECTRE_SCRIPT_ARGUMENT_TYPE_ENTITY: {
      flecs::entity resolved_entity =
          resolve_entity_argument(target_entity, argument.value.entity);
...
void script_module_t::execute_on_create(flecs::entity target_entity) {
  ...
  scripts_entity.each<spectre_use_script_on_create_relation_t>(
      [&](flecs::entity script_entity) {
        const auto *relation =
            scripts_entity.try_get<spectre_use_script_on_create_relation_t>(
                script_entity);
        if (relation)
          execute_script_with_target(target_entity, script_entity,
                                     relation->arguments,
                                     relation->argument_count);
      });
}
...
void script_module_t::execute_on_destroy(flecs::entity target_entity) {
  ...
  if (!scripts_entity.is_valid())
    scripts_entity = target_entity;
  scripts_entity.each<spectre_use_script_on_destroy_relation_t>(
      [&](flecs::entity script_entity) {
        const auto *relation =
            scripts_entity.try_get<spectre_use_script_on_destroy_relation_t>(
                script_entity);
        if (relation)
          execute_script_with_target(target_entity, script_entity,
                                     relation->arguments,
...
void script_module_t::execute_on_update(flecs::entity target_entity) {
  ...
  if (!scripts_entity.is_valid())
    scripts_entity = target_entity;
  scripts_entity.each<spectre_use_script_on_update_relation_t>(
      [&](flecs::entity script_entity) {
        const auto *relation =
            scripts_entity.try_get<spectre_use_script_on_update_relation_t>(
                script_entity);
        if (relation)
          execute_script_with_target(target_entity, script_entity,
                                     relation->arguments,
...
void script_module_t::execute_on_enter(flecs::entity target_entity) {
  ...
  if (!scripts_entity.is_valid())
    scripts_entity = target_entity;
  scripts_entity.each<spectre_use_script_on_enter_relation_t>(
      [&](flecs::entity script_entity) {
        const auto *relation =
            scripts_entity.try_get<spectre_use_script_on_enter_relation_t>(
                script_entity);
        if (relation)
          execute_script_with_target(target_entity, script_entity,
                                     relation->arguments,
...
void script_module_t::execute_on_exit(flecs::entity target_entity) {
  ...
  if (!scripts_entity.is_valid())
    scripts_entity = target_entity;
  scripts_entity.each<spectre_use_script_on_exit_relation_t>(
      [&](flecs::entity script_entity) {
        const auto *relation =
            scripts_entity.try_get<spectre_use_script_on_exit_relation_t>(
                script_entity);
        if (relation)
          execute_script_with_target(target_entity, script_entity,
                                     relation->arguments,
