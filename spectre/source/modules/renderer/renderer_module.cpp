...
static void
deserialize_renderer_cb(ecs_world_t *world, ecs_entity_t entity,
                        sandbox_properties_handle_t properties_handle) {
  spectre::modules::renderer::deserialize_renderer(flecs::world(world), entity,
                                                   properties_handle);
}

static sandbox_properties_handle_t
serialize_renderer_cb(ecs_world_t *world, ecs_entity_t entity_id) {
  return spectre::modules::renderer::serialize_renderer(flecs::world(world),
                                                        entity_id);
}

static ecs_entity_t register_renderable_comp(ecs_world_t *world) {
  return flecs::world(world)
      .component<spectre_renderable_t>()
      .member<char>("dummy")
      .id();
}
...
static spectre_color_t deserialize_color(sandbox::properties props) {
  ...
  std::cout << "deserialize_color parsed: r=" << color.r << ", g=" << color.g
...
// Serializers
static sandbox_properties_handle_t
serialize_rectangle_renderable(ecs_world_t *world, ecs_entity_t entity) {
  if (!world || !entity)
    return {0};
  flecs::world flecs_world(world);
  const auto *comp =
      flecs_world.entity(entity).try_get<spectre_rectange_renderable_t>();
  if (!comp)
    return {0};
  sandbox::properties props;
...
static void
deserialize_rectangle_renderable(ecs_world_t *world, ecs_entity_t entity,
...
static sandbox_properties_handle_t
serialize_circle_renderable(ecs_world_t *world, ecs_entity_t entity) {
  if (!world || !entity)
    return {0};
  flecs::world flecs_world(world);
  const auto *comp =
      flecs_world.entity(entity).try_get<spectre_circle_renderable_t>();
  if (!comp)
    return {0};
  sandbox::properties props;
...
static void deserialize_circle_renderable(ecs_world_t *world,
                                          ecs_entity_t entity,
                                          sandbox_properties_handle_t handle) {
  if (!world)
    return;
...
static sandbox_properties_handle_t
serialize_polygon_renderable(ecs_world_t *world, ecs_entity_t entity) {
  if (!world || !entity)
    return {0};
  flecs::world flecs_world(world);
  const auto *comp =
      flecs_world.entity(entity).try_get<spectre_polygone_renderable_t>();
  if (!comp)
    return {0};
  sandbox::properties props;
  props.set("radius", comp->radius);
...
static void deserialize_polygon_renderable(ecs_world_t *world,
                                           ecs_entity_t entity,
                                           sandbox_properties_handle_t handle) {
  if (!world)
    return;
...
static sandbox_properties_handle_t
serialize_line_renderable(ecs_world_t *world, ecs_entity_t entity) {
  if (!world || !entity)
    return {0};
  flecs::world flecs_world(world);
  const auto *comp =
      flecs_world.entity(entity).try_get<spectre_ligne_renderable_t>();
  if (!comp)
    return {0};
  sandbox::properties props;
...
static void deserialize_line_renderable(ecs_world_t *world, ecs_entity_t entity,
                                        sandbox_properties_handle_t handle) {
  if (!world)
    return;
  sandbox::properties props(handle, false);
...
static sandbox_properties_handle_t serialize_empty(ecs_world_t *,
                                                   ecs_entity_t) {
  return {0};
}
static void deserialize_empty(ecs_world_t *, ecs_entity_t,
                              sandbox_properties_handle_t) {}

static sandbox_properties_handle_t serialize_renderable(ecs_world_t *world,
                                                        ecs_entity_t entity) {
  return {0};
}
static void deserialize_renderable(ecs_world_t *world, ecs_entity_t entity,
                                   sandbox_properties_handle_t) {
  if (world && entity) {
    flecs::world flecs_world(world);
...
static sandbox_properties_handle_t
serialize_2D_transform_component(ecs_world_t *world, ecs_entity_t entity) {
  if (!world || !entity)
    return {0};
  flecs::world flecs_world(world);
  const auto *comp =
      flecs_world.entity(entity).try_get<spectre_2D_transform_component_t>();
  if (!comp)
    return {0};
  sandbox::properties props;
...
static void
deserialize_2D_transform_component(ecs_world_t *world, ecs_entity_t entity,
                                   sandbox_properties_handle_t handle) {
  if (!world)
    return;
  sandbox::properties props(handle, false);
...
renderer_module_t::renderer_module_t(flecs::world &world) : m_world(world) {
  ...
  spectre::modules::serializer::register_serializer(m_world, "renderer",
                                                    &serializer_callbacks);

  // Register Renderer Components via SDK
  spectre_serializer_component empty_serializer = {deserialize_empty,
                                                   serialize_empty};
  spectre_serializer_component renderable_serializer = {deserialize_renderable,
                                                        serialize_renderable};

  spectre_serializer_component rect_serializer = {
      deserialize_rectangle_renderable, serialize_rectangle_renderable};
  spectre_serializer_component poly_serializer = {
      deserialize_polygon_renderable, serialize_polygon_renderable};
  spectre_serializer_component line_serializer = {deserialize_line_renderable,
                                                  serialize_line_renderable};
  spectre_serializer_component transform_serializer = {
      deserialize_2D_transform_component, serialize_2D_transform_component};

  register_renderable_comp(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_renderable_t", &renderable_serializer);

  register_rectangle_comp(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_rectange_renderable_t", &rect_serializer);

  register_circle_comp(m_world.c_ptr());
  spectre_serializer_component circle_serializer = {
      deserialize_circle_renderable, serialize_circle_renderable};
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_circle_renderable_t", &circle_serializer);

  register_polygon_comp(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_polygone_renderable_t", &poly_serializer);

  register_custom_polygon_comp(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_custom_polygone_renderable_t", &empty_serializer);

  register_line_comp(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_ligne_renderable_t", &line_serializer);
  ...
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_2D_transform_component_t", &transform_serializer);
  ...
  sandbox::modules::logs::info(m_world,
                               "[Renderer Module] Initialized successfully.");
}

renderer_module_t::~renderer_module_t() = default;

void renderer_module_t::deserialize_renderer(
    flecs::entity target, const sandbox::properties &properties) {
  sandbox::modules::logs::debug(
      const_cast<flecs::world &>(m_world),
      "[Renderer Module] Deserializing renderer entity.");
}

sandbox::properties
renderer_module_t::serialize_renderer(flecs::entity renderer_entity) {
  return sandbox::properties{};
}

void renderer_module_t::register_renderer(
    const sandbox::properties &properties) {
  m_renderer =
      m_world.entity("::renderer").add<spectre_renderer_update_marker_t>();
...
bool renderer_module_t::is_renderer() const { return m_renderer.is_valid(); }

void renderer_module_t::render_frame() {
  BeginDrawing();
  ClearBackground(
      RAYWHITE); // Or any default color, let's use a standard default

  ecs_entity_t current_state_id =
      spectre::modules::scenes::find_current_state(m_world);
  ...
  if (!current_state.is_valid()) {
    ...
    renderable_query.each(
        [this](flecs::entity entity, spectre_renderable_t &renderable) {
          this->render(entity);
        });
    EndDrawing();
    return;
  ...
  renderable_query.each([&](flecs::entity entity, spectre_renderable_t &) {
    ...
    while (parent_entity.is_valid() && parent_entity.id() != 0) {
      if (parent_entity.has<spectre_state_use_scene_relation_t>() &&
        ...
        layer_index =
            parent_entity.get<spectre_state_use_scene_relation_t>().layer_index;
  ...
  std::sort(entities_to_render.begin(), entities_to_render.end(),
            [](const renderable_entity_t &entity_a,
               const renderable_entity_t &entity_b) {
...
void renderer_module_t::render(flecs::entity entity_to_render) {
  ...
  if (entity_to_render.has<spectre_2D_transform_component_t>()) {
    const auto *transform =
        entity_to_render.try_get<spectre_2D_transform_component_t>();
    if (transform) {
  ...
  if (entity_to_render.has<spectre_rectange_renderable_t>()) {
    const auto *rect =
        entity_to_render.try_get<spectre_rectange_renderable_t>();
    if (rect) {
      DrawRectangle(0, 0, (int)rect->width, (int)rect->height,
                    to_raylib_color(rect->fill_color));
      if (rect->outline_thickness > 0) {
        DrawRectangleLinesEx(Rectangle{0, 0, rect->width, rect->height},
  ...
  if (entity_to_render.has<spectre_circle_renderable_t>()) {
    const auto *circle =
        entity_to_render.try_get<spectre_circle_renderable_t>();
    if (circle) {
      DrawCircleV(Vector2{0.0f, 0.0f}, circle->radius,
                  to_raylib_color(circle->fill_color));
      if (circle->outline_thickness > 0) {
        // raylib DrawCircleLines lacks thickness, but we can draw a ring
        DrawRing(Vector2{0.0f, 0.0f},
                 circle->radius - circle->outline_thickness, circle->radius, 0,
                 360, 36, to_raylib_color(circle->outline_color));
      }
    }
  }
  ...
  if (entity_to_render.has<spectre_polygone_renderable_t>()) {
    const auto *poly =
        entity_to_render.try_get<spectre_polygone_renderable_t>();
    if (poly) {
      DrawPoly(Vector2{0.0f, 0.0f}, poly->point_count, poly->radius, 0.0f,
               to_raylib_color(poly->fill_color));
      if (poly->outline_thickness > 0) {
        DrawPolyLinesEx(Vector2{0.0f, 0.0f}, poly->point_count, poly->radius,
                        0.0f, poly->outline_thickness,
  ...
  if (entity_to_render.has<spectre_custom_polygone_renderable_t>()) {
    const auto *cpoly =
        entity_to_render.try_get<spectre_custom_polygone_renderable_t>();
    if (cpoly && cpoly->vertices && cpoly->vertex_count >= 3) {
      ...
      DrawTriangleFan(points.data(), points.size(),
                      to_raylib_color(cpoly->fill_color));

      if (cpoly->outline_thickness > 0) {
        for (uint32_t i = 0; i < cpoly->vertex_count; ++i) {
          Vector2 p1 = points[i];
          Vector2 p2 = points[(i + 1) % cpoly->vertex_count];
          DrawLineEx(p1, p2, cpoly->outline_thickness,
                     to_raylib_color(cpoly->outline_color));
        }
      }
    }
  }
  ...
  if (entity_to_render.has<spectre_ligne_renderable_t>()) {
    ...
    if (line) {
      ...
      DrawLineEx(startPos, endPos, line->thickness,
