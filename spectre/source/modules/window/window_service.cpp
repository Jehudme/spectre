...
ecs_entity_t spectre_window_deserialize_window(ecs_world_t* entity_world, sandbox_properties_handle_t props) {
    if (!entity_world) return 0;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->deserialize_window) {
            return service->api->deserialize_window(entity_world, props);
        }
    }
    return 0;
}

sandbox_properties_handle_t spectre_window_serialize_window(ecs_world_t* entity_world, ecs_entity_t window) {
    if (!entity_world) return {0};
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->serialize_window) {
            return service->api->serialize_window(entity_world, window);
        }
    }
    return {0};
}

void spectre_window_register_window(ecs_world_t* entity_world, sandbox_properties_handle_t props) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->register_window) {
            service->api->register_window(entity_world, props);
        }
    }
}

bool spectre_window_should_close(ecs_world_t* entity_world) {
    if (!entity_world) return false;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->should_close) {
            return service->api->should_close(entity_world);
        }
    }
    return false;
}

void spectre_window_set_should_close(ecs_world_t* entity_world, bool close) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->set_should_close) {
            service->api->set_should_close(entity_world, close);
        }
    }
}

void spectre_window_set_size(ecs_world_t* entity_world, uint32_t width, uint32_t height) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->set_size) {
            service->api->set_size(entity_world, width, height);
        }
    }
}

void spectre_window_set_position(ecs_world_t* entity_world, int32_t x, int32_t y) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->set_position) {
            service->api->set_position(entity_world, x, y);
        }
    }
}

void spectre_window_set_size_limits(ecs_world_t* entity_world, uint32_t min_width, uint32_t min_height, uint32_t max_width, uint32_t max_height) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->set_size_limits) {
            service->api->set_size_limits(entity_world, min_width, min_height, max_width, max_height);
        }
    }
}

uint32_t spectre_window_get_width(ecs_world_t* entity_world) {
    if (!entity_world) return 0;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->get_width) {
            return service->api->get_width(entity_world);
        }
    }
    return 0;
}

uint32_t spectre_window_get_height(ecs_world_t* entity_world) {
    if (!entity_world) return 0;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->get_height) {
            return service->api->get_height(entity_world);
        }
    }
    return 0;
}

float spectre_window_get_aspect_ratio(ecs_world_t* entity_world) {
    if (!entity_world) return 0;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->get_aspect_ratio) {
            return service->api->get_aspect_ratio(entity_world);
        }
    }
    return 0;
}

int32_t spectre_window_get_position_x(ecs_world_t* entity_world) {
    if (!entity_world) return 0;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->get_position_x) {
            return service->api->get_position_x(entity_world);
        }
    }
    return 0;
}

int32_t spectre_window_get_position_y(ecs_world_t* entity_world) {
    if (!entity_world) return 0;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->get_position_y) {
            return service->api->get_position_y(entity_world);
        }
    }
    return 0;
}

void spectre_window_set_title(ecs_world_t* entity_world, const char* title) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->set_title) {
            service->api->set_title(entity_world, title);
        }
    }
}

void spectre_window_set_vsync(ecs_world_t* entity_world, bool enabled) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->set_vsync) {
            service->api->set_vsync(entity_world, enabled);
        }
    }
}

void spectre_window_set_fullscreen(ecs_world_t* entity_world, bool fullscreen) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->set_fullscreen) {
            service->api->set_fullscreen(entity_world, fullscreen);
        }
    }
}

void spectre_window_set_borderless(ecs_world_t* entity_world, bool borderless) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->set_borderless) {
            service->api->set_borderless(entity_world, borderless);
        }
    }
}

void spectre_window_set_resizable(ecs_world_t* entity_world, bool resizable) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->set_resizable) {
            service->api->set_resizable(entity_world, resizable);
        }
    }
}

void spectre_window_set_always_on_top(ecs_world_t* entity_world, bool always_on_top) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->set_always_on_top) {
            service->api->set_always_on_top(entity_world, always_on_top);
        }
    }
}

const char* spectre_window_get_title(ecs_world_t* entity_world) {
    if (!entity_world) return 0;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->get_title) {
            return service->api->get_title(entity_world);
        }
    }
    return 0;
}

bool spectre_window_is_vsync(ecs_world_t* entity_world) {
    if (!entity_world) return false;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->is_vsync) {
            return service->api->is_vsync(entity_world);
        }
    }
    return false;
}

bool spectre_window_is_fullscreen(ecs_world_t* entity_world) {
    if (!entity_world) return false;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->is_fullscreen) {
            return service->api->is_fullscreen(entity_world);
        }
    }
    return false;
}

bool spectre_window_is_borderless(ecs_world_t* entity_world) {
    if (!entity_world) return false;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->is_borderless) {
            return service->api->is_borderless(entity_world);
        }
    }
    return false;
}

bool spectre_window_is_resizable(ecs_world_t* entity_world) {
    if (!entity_world) return false;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->is_resizable) {
            return service->api->is_resizable(entity_world);
        }
    }
    return false;
}

void spectre_window_minimize(ecs_world_t* entity_world) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->minimize) {
            service->api->minimize(entity_world);
        }
    }
}

void spectre_window_maximize(ecs_world_t* entity_world) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->maximize) {
            service->api->maximize(entity_world);
        }
    }
}

void spectre_window_restore(ecs_world_t* entity_world) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->restore) {
            service->api->restore(entity_world);
        }
    }
}

void spectre_window_show(ecs_world_t* entity_world) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->show) {
            service->api->show(entity_world);
        }
    }
}

void spectre_window_hide(ecs_world_t* entity_world) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->hide) {
            service->api->hide(entity_world);
        }
    }
}

void spectre_window_request_attention(ecs_world_t* entity_world) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->request_attention) {
            service->api->request_attention(entity_world);
        }
    }
}

bool spectre_window_is_minimized(ecs_world_t* entity_world) {
    if (!entity_world) return false;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->is_minimized) {
            return service->api->is_minimized(entity_world);
        }
    }
    return false;
}

bool spectre_window_is_maximized(ecs_world_t* entity_world) {
    if (!entity_world) return false;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->is_maximized) {
            return service->api->is_maximized(entity_world);
        }
    }
    return false;
}

bool spectre_window_is_visible(ecs_world_t* entity_world) {
    if (!entity_world) return false;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->is_visible) {
            return service->api->is_visible(entity_world);
        }
    }
    return false;
}

bool spectre_window_is_focused(ecs_world_t* entity_world) {
    if (!entity_world) return false;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->is_focused) {
            return service->api->is_focused(entity_world);
        }
    }
    return false;
}

bool spectre_window_is_hovered(ecs_world_t* entity_world) {
    if (!entity_world) return false;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->is_hovered) {
            return service->api->is_hovered(entity_world);
        }
    }
    return false;
}

void spectre_window_set_cursor_visible(ecs_world_t* entity_world, bool visible) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->set_cursor_visible) {
            service->api->set_cursor_visible(entity_world, visible);
        }
    }
}

void spectre_window_set_cursor_locked(ecs_world_t* entity_world, bool locked) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->set_cursor_locked) {
            service->api->set_cursor_locked(entity_world, locked);
        }
    }
}

bool spectre_window_is_cursor_visible(ecs_world_t* entity_world) {
    if (!entity_world) return false;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->is_cursor_visible) {
            return service->api->is_cursor_visible(entity_world);
        }
    }
    return false;
}

bool spectre_window_is_cursor_locked(ecs_world_t* entity_world) {
    if (!entity_world) return false;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->is_cursor_locked) {
            return service->api->is_cursor_locked(entity_world);
        }
    }
    return false;
}

void* spectre_window_get_native_handle(ecs_world_t* entity_world) {
    if (!entity_world) return 0;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->get_native_handle) {
            return service->api->get_native_handle(entity_world);
        }
    }
    return 0;
}

bool spectre_window_is_key_down(ecs_world_t* entity_world, int keycode) {
    if (!entity_world) return false;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->is_key_down) {
            return service->api->is_key_down(entity_world, keycode);
        }
    }
    return false;
}

bool spectre_window_is_key_pressed(ecs_world_t* entity_world, int keycode) {
    if (!entity_world) return false;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->is_key_pressed) {
            return service->api->is_key_pressed(entity_world, keycode);
        }
    }
    return false;
}

bool spectre_window_is_key_released(ecs_world_t* entity_world, int keycode) {
    if (!entity_world) return false;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->is_key_released) {
            return service->api->is_key_released(entity_world, keycode);
        }
    }
    return false;
}

float spectre_window_get_mouse_x(ecs_world_t* entity_world) {
    if (!entity_world) return 0;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->get_mouse_x) {
            return service->api->get_mouse_x(entity_world);
        }
    }
    return 0;
}

float spectre_window_get_mouse_y(ecs_world_t* entity_world) {
    if (!entity_world) return 0;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->get_mouse_y) {
            return service->api->get_mouse_y(entity_world);
        }
    }
    return 0;
}

float spectre_window_get_mouse_delta_x(ecs_world_t* entity_world) {
    if (!entity_world) return 0;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->get_mouse_delta_x) {
            return service->api->get_mouse_delta_x(entity_world);
        }
    }
    return 0;
}

float spectre_window_get_mouse_delta_y(ecs_world_t* entity_world) {
    if (!entity_world) return 0;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_window_service_t)) {
        if (service->api && service->api->get_mouse_delta_y) {
            return service->api->get_mouse_delta_y(entity_world);
        }
    }
    return 0;
}
