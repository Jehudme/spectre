...
static void window_set_size_limits(ecs_world_t* entity_world, uint32_t min_width, uint32_t min_height,
...
static void window_deserialize_window(ecs_world_t* world, ecs_entity_t target, sandbox_properties_handle_t props) {
    ...
    auto* module = flecs_world.lookup("spectre::modules::window_module_t").is_valid()
                       ? flecs_world.try_get_mut<spectre::modules::window_module_t>()
                       : nullptr;
    if (module)
        module->deserialize_window(flecs_world.entity(target), sandbox::properties(props, false));
}
...
static sandbox_properties_handle_t window_serialize_window(ecs_world_t* entity_world, ecs_entity_t window) {
    ...
    auto* module = flecs_world.lookup("spectre::modules::window_module_t").is_valid()
                       ...
                       : nullptr;
    if (module) {
        sandbox::properties props = module->serialize_window(flecs_world.entity(window));
...
static void window_set_size_limits(ecs_world_t* entity_world, uint32_t min_w, uint32_t min_h, uint32_t max_w,
                                   uint32_t max_h) {
    if (!entity_world)
        return;
    flecs::world flecs_world(entity_world);
...
#else
    const spectre_window_service_t* service = (const spectre_window_service_t*)ecs_singleton_get(
...
void window::set_size_limits(const flecs::world& entity_world, uint32_t min_width, uint32_t min_height,
                             uint32_t max_width, uint32_t max_height) {
