...
namespace spectre::modules {
    ...
    static ecs_entity_t register_window_comp(ecs_world_t* world) { return flecs::world(world).component<spectre_window_component_t>().id(); }
    static ecs_entity_t register_input_state_comp(ecs_world_t* world) { return flecs::world(world).component<spectre_input_state_t>().id(); }
    ...
    window_module_t::window_module_t(flecs::world& world) : m_world(world) {
        ...
        m_world.entity<spectre_on_event_phase_t>("spectre_on_event_phase")
               .add(flecs::Phase)
    ...
    sandbox::properties window_module_t::serialize_window(flecs::entity window_entity) {
        ...
        if (!window_entity.is_valid() || !window_entity.has<spectre_window_component_t>()) return properties;
    ...
    void window_module_t::register_window(const sandbox::properties& properties) {
        ...
        if (m_window_entity.is_valid()) {
            ...
            if (component) {
                ...
                if (component->vsync) flags |= FLAG_VSYNC_HINT;
                if (component->fullscreen) flags |= FLAG_FULLSCREEN_MODE;
                if (component->borderless) flags |= FLAG_WINDOW_UNDECORATED;
                if (component->resizable) flags |= FLAG_WINDOW_RESIZABLE;
                if (component->always_on_top) flags |= FLAG_WINDOW_TOPMOST;
                if (component->minimized) flags |= FLAG_WINDOW_MINIMIZED;
                if (component->maximized) flags |= FLAG_WINDOW_MAXIMIZED;
                if (!component->visible) flags |= FLAG_WINDOW_HIDDEN;
    ...
    void window_module_t::set_should_close(bool close) {
        sandbox::modules::logs::warn(const_cast<flecs::world&>(m_world), "[Window Module] set_should_close is not supported natively in Raylib.");
    }
    ...
    void window_module_t::request_attention() {
        sandbox::modules::logs::warn(const_cast<flecs::world&>(m_world), "[Window Module] request_attention is not supported natively in Raylib.");
    }
    ...
    bool window_module_t::is_key_down(int keycode) const {
        if (!m_window_entity.is_valid() || keycode < 0 || keycode >= 512) return false;
    ...
    float window_module_t::get_mouse_x() const {
        if (!m_window_entity.is_valid()) return 0.0f;
        const auto* input_state = m_window_entity.try_get<spectre_input_state_t>();
    ...
    void window_module_t::apply_window_change() {
        ...
        SetWindowMinSize(component->min_width, component->min_height);
        SetWindowMaxSize(component->max_width, component->max_height);
        if (component->title) SetWindowTitle(component->title);

        if (component->vsync) SetWindowState(FLAG_VSYNC_HINT); else ClearWindowState(FLAG_VSYNC_HINT);
        if (component->fullscreen) SetWindowState(FLAG_FULLSCREEN_MODE); else ClearWindowState(FLAG_FULLSCREEN_MODE);
        if (component->borderless) SetWindowState(FLAG_WINDOW_UNDECORATED); else ClearWindowState(FLAG_WINDOW_UNDECORATED);
        if (component->resizable) SetWindowState(FLAG_WINDOW_RESIZABLE); else ClearWindowState(FLAG_WINDOW_RESIZABLE);
        if (component->always_on_top) SetWindowState(FLAG_WINDOW_TOPMOST); else ClearWindowState(FLAG_WINDOW_TOPMOST);
        if (component->minimized) MinimizeWindow(); else if (component->maximized) MaximizeWindow(); else RestoreWindow();
        if (component->visible) ClearWindowState(FLAG_WINDOW_HIDDEN); else SetWindowState(FLAG_WINDOW_HIDDEN);
        
        if (component->cursor_visible) ShowCursor(); else HideCursor();
        if (component->cursor_locked) DisableCursor(); else EnableCursor();
    }
    ...
    void window_module_t::set_key_down(int keycode) {
        sandbox::modules::logs::warn(const_cast<flecs::world&>(m_world), "[Window Module] set_key_down is not supported natively. Input is managed by Raylib.");
    }

    void window_module_t::set_key_up(int keycode) {
        sandbox::modules::logs::warn(const_cast<flecs::world&>(m_world), "[Window Module] set_key_up is not supported natively. Input is managed by Raylib.");
    }

    void window_module_t::set_mouse_position(float position_x, float position_y) {
        if (IsWindowReady()) SetMousePosition(static_cast<int>(position_x), static_cast<int>(position_y));
    }

    void window_module_t::set_mouse_delta(float delta_x, float delta_y) {
        sandbox::modules::logs::warn(const_cast<flecs::world&>(m_world), "[Window Module] set_mouse_delta is not supported natively. Input is managed by Raylib.");
    }
