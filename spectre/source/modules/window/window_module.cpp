...
static void
deserialize_window_cb(ecs_world_t *world, ecs_entity_t entity,
                      sandbox_properties_handle_t properties_handle);
static sandbox_properties_handle_t serialize_window_cb(ecs_world_t *world,
...
window_module_t::window_module_t(flecs::world &world) : m_world(world) {
  ...
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_window_component_t", &empty_serializer);

  register_input_state_comp(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_input_state_t", &empty_serializer);
  ...
  spectre::modules::serializer::register_serializer(m_world, "window",
                                                    &window_serializer);
  ...
  sandbox::modules::logs::info(const_cast<flecs::world &>(m_world),
                               "[Window Module] Initialized successfully.");
}
...
deserialize_window_cb(ecs_world_t *world, ecs_entity_t entity,
  ...
  if (module_instance) {
    ...
    module_instance->deserialize_window(flecs_world.entity(entity),
                                        parsed_properties);
  }
}
...
static sandbox_properties_handle_t serialize_window_cb(ecs_world_t *world,
  ...
  if (module_instance) {
    sandbox::properties serialized_properties =
        module_instance->serialize_window(flecs_world.entity(entity_id));
...
void window_module_t::deserialize_window(
    flecs::entity window_entity, const sandbox::properties &properties) {
  ...
  std::string title =
      properties.get<std::string>("title").value_or("Spectre Engine");
  ...
  component.always_on_top =
      properties.get<bool>("always_on_top").value_or(false);
  ...
  component.cursor_visible =
      properties.get<bool>("cursor_visible").value_or(true);
  component.cursor_locked =
      properties.get<bool>("cursor_locked").value_or(false);
...
sandbox::properties
window_module_t::serialize_window(flecs::entity window_entity) {
  sandbox::properties properties;
  if (!window_entity.is_valid() ||
      !window_entity.has<spectre_window_component_t>())
    return properties;
  ...
  properties.set("title",
                 std::string(component->title ? component->title : ""));
...
void window_module_t::register_window(const sandbox::properties &properties) {
  ...
  if (m_window_entity.is_valid()) {
    const auto *component =
        m_window_entity.try_get<spectre_window_component_t>();
    if (component) {
      ...
      InitWindow(component->width, component->height,
                 component->title ? component->title : "Spectre Engine");
...
bool window_module_t::should_close() const { return WindowShouldClose(); }
...
void window_module_t::set_size_limits(uint32_t min_width, uint32_t min_height,
...
float window_module_t::get_aspect_ratio() const {
  ...
  if (component && component->height != 0) {
    return static_cast<float>(component->width) /
           static_cast<float>(component->height);
  }
  return 0.0f;
}
...
bool window_module_t::is_key_down(int keycode) const {
  ...
  if (input_state) {
    return (input_state->keys_down[keycode / 64] & (1ULL << (keycode % 64))) !=
           0;
  }
  return false;
}
...
bool window_module_t::is_key_pressed(int keycode) const {
  ...
  const auto *input_state = m_window_entity.try_get<spectre_input_state_t>();
  if (input_state) {
    return (input_state->keys_pressed[keycode / 64] &
            (1ULL << (keycode % 64))) != 0;
  }
  return false;
}
...
bool window_module_t::is_key_released(int keycode) const {
  ...
  const auto *input_state = m_window_entity.try_get<spectre_input_state_t>();
  if (input_state) {
    return (input_state->keys_released[keycode / 64] &
            (1ULL << (keycode % 64))) != 0;
  }
  return false;
}
...
void window_module_t::begin_input_frame() {
  ...
  if (input_state) {
    ...
    std::memset(input_state->keys_pressed, 0,
                sizeof(input_state->keys_pressed));
    std::memset(input_state->keys_released, 0,
                sizeof(input_state->keys_released));
