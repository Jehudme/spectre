...
static sandbox_properties_handle_t serialize_state_cb(ecs_world_t *world,
                                                      ecs_entity_t entity_id) {
  return spectre::modules::scenes::serialize_state(flecs::world(world),
                                                   entity_id);
}

static void
deserialize_state_cb(ecs_world_t *world, ecs_entity_t entity,
                     sandbox_properties_handle_t properties_handle) {
  spectre::modules::scenes::deserialize_state(flecs::world(world), entity,
                                              properties_handle);
}

static sandbox_properties_handle_t serialize_scene_cb(ecs_world_t *world,
                                                      ecs_entity_t entity_id) {
  return spectre::modules::scenes::serialize_scene(flecs::world(world),
                                                   entity_id);
}

static void
deserialize_scene_cb(ecs_world_t *world, ecs_entity_t entity,
                     sandbox_properties_handle_t properties_handle) {
  spectre::modules::scenes::deserialize_scene(flecs::world(world), entity,
                                              properties_handle);
}

static ecs_entity_t register_scene_component(ecs_world_t *world) {
  return flecs::world(world)
      .component<spectre_scene_t>()
      .member<char>("dummy")
      .id();
}
static ecs_entity_t register_state_component(ecs_world_t *world) {
  return flecs::world(world)
      .component<spectre_state_t>()
      .member<char>("dummy")
      .id();
}
static ecs_entity_t
register_state_use_scene_relation_component(ecs_world_t *world) {
  return flecs::world(world)
      .component<spectre_state_use_scene_relation_t>()
      .member<int>("layer_index")
      .id();
}

static ecs_entity_t register_state_context_component(ecs_world_t *world) {
  return flecs::world(world)
      .component<spectre_state_context_t>()
      .id(); // holds flecs::entity, reflection is tricky
}
...
static ecs_entity_t register_disable_rendering_component(ecs_world_t *world) {
  return flecs::world(world)
      .component<spectre_disable_rendering_t>()
      .member<char>("dummy")
      .id();
}
...
scenes_module_t::scenes_module_t(flecs::world &world) : m_world(world) {
  ...
  spectre::modules::serializer::register_serializer(m_world, "spectre_scene_t",
                                                    &empty_serializer);
  register_state_component(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(m_world, "spectre_state_t",
                                                    &empty_serializer);
  register_state_use_scene_relation_component(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_state_use_scene_relation_t", &empty_serializer);
  register_state_context_component(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_state_context_t", &empty_serializer);
  register_scene_context_component(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_scene_context_t", &empty_serializer);
  register_disable_rendering_component(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_disable_rendering_t", &empty_serializer);
  ...
  spectre::modules::serializer::register_serializer(m_world, "state",
                                                    &state_serializer);
  ...
  spectre::modules::serializer::register_serializer(m_world, "scene",
                                                    &scene_serializer);

  m_scene_serializer = m_world.entity(
      spectre::modules::serializer::find_serializer(m_world, "scene"));
  m_state_serializer = m_world.entity(
      spectre::modules::serializer::find_serializer(m_world, "state"));
  m_script_args_serializer = m_world.entity(
      spectre::modules::serializer::find_serializer(m_world, "scripts"));
  m_entity_serializer = m_world.entity(
      spectre::modules::serializer::find_serializer(m_world, "entity"));

  if (!m_script_args_serializer.is_valid()) {
    sandbox::modules::logs::warn(
        const_cast<flecs::world &>(m_world),
        "[Scenes Module] Could not find 'scripts' serializer. Serialization "
        "might be incomplete.");
  }
  if (!m_entity_serializer.is_valid()) {
    sandbox::modules::logs::warn(
        const_cast<flecs::world &>(m_world),
        "[Scenes Module] Could not find 'entity' serializer. Serialization "
        "might be incomplete.");
  }
  ...
  m_world.system<spectre_scenes_update_marker_t>("Scenes OnUpdate")
      ...
      .each([this](flecs::entity entity, spectre_scenes_update_marker_t) {
        ...
        if (current_state.is_valid() &&
            !current_state.has<spectre_disable_rendering_t>()) {
          ...
          current_state.children([&](flecs::entity scene_entity) {
            if (scene_entity.has<spectre_state_use_scene_relation_t>()) {
              ...
              scene_entity.children([&](flecs::entity child_entity) {
                if (!child_entity.has<spectre_scene_t>() &&
                    !child_entity.has<spectre_state_t>()) {
  ...
  sandbox::modules::logs::info(const_cast<flecs::world &>(m_world),
                               "[Scenes Module] Initialized successfully.");
}

scenes_module_t::~scenes_module_t() = default;

sandbox::properties
scenes_module_t::serialize_state(flecs::entity state_entity) {
  sandbox::properties properties;
  if (!state_entity.is_valid())
    return properties;
  ...
  if (m_script_args_serializer != 0) {
    sandbox_properties_handle_t properties_handle =
        spectre::modules::serializer::serialize_entity(
            m_world, m_script_args_serializer, state_entity.id());
  ...
  state_entity.children([&](flecs::entity child_entity) {
    if (is_scene(child_entity)) {
      scenes_array.set<std::string>(std::to_string(layer_index++),
...
void scenes_module_t::deserialize_state(flecs::entity state_entity,
                                        sandbox::properties properties) {
  if (!properties.is_valid() || !state_entity.is_valid())
    return;
  ...
  if (properties.has("scripts") && m_script_args_serializer != 0) {
    ...
    spectre::modules::serializer::deserialize_entity(
        m_world, m_script_args_serializer, state_entity.id(),
    ...
    if (properties.get_array<std::string>("scenes", scenes_array)) {
      for (size_t index = 0; index < scenes_array.size(); ++index) {
        ...
        if (scene_prefab.is_valid()) {
          flecs::entity scene_instance =
              m_world.prefab().is_a(scene_prefab).child_of(state_entity);
          scene_instance.set<spectre_state_use_scene_relation_t>(
              {static_cast<int>(index)});
          sandbox::modules::logs::info(
              const_cast<flecs::world &>(m_world),
              "[Scenes Module] Attached scene '{}' to state.", scene_name);
        } else {
          sandbox::modules::logs::warn(const_cast<flecs::world &>(m_world),
                                       "[Scenes Module] Referenced scene '{}' "
                                       "not found when deserializing state.",
                                       scene_name);
        }
      }
    } else {
      sandbox::modules::logs::error(
          const_cast<flecs::world &>(m_world),
          "[Scenes Module] Failed to parse 'scenes' array.");
    }
  }
}

sandbox::properties
scenes_module_t::serialize_scene(flecs::entity scene_entity) {
  sandbox::properties properties;
    ...
    scene_entity.children([&](flecs::entity child_entity) {
      if (child_entity.name() != "scripts") {
        ...
        if (child_properties.is_valid()) {
          hierarchies_array.merge(std::to_string(child_index++),
                                  child_properties);
        }
      }
    });
...
void scenes_module_t::deserialize_scene(flecs::entity scene_entity,
                                        sandbox::properties properties) {
  if (!properties.is_valid() || !scene_entity.is_valid())
    return;
...
void scenes_module_t::register_state(sandbox::properties properties) {
  ...
  state_prefab.children(
      [&](flecs::entity child_entity) { child_entity.destruct(); });

  deserialize_state(state_prefab, std::move(properties));
  sandbox::modules::logs::info(const_cast<flecs::world &>(m_world),
                               "[Scenes Module] Registered state '{}'.",
                               state_name);
}
...
void scenes_module_t::register_scene(sandbox::properties properties) {
  ...
  if (!properties.get<std::string>("name", scene_name) || scene_name.empty()) {
    sandbox::modules::logs::error(
        const_cast<flecs::world &>(m_world),
        "[Scenes Module] Cannot register scene with empty name.");
    return;
  }
  ...
  scene_prefab.children(
      [&](flecs::entity child_entity) { child_entity.destruct(); });

  deserialize_scene(scene_prefab, std::move(properties));
  sandbox::modules::logs::info(const_cast<flecs::world &>(m_world),
                               "[Scenes Module] Registered scene '{}'.",
                               scene_name);
}
...
bool scenes_module_t::has_state(std::string_view state_name) const {
  ...
  flecs::entity state_entity =
      m_states_root.lookup(std::string(state_name).c_str());
...
bool scenes_module_t::has_scene(std::string_view scene_name) const {
  ...
  flecs::entity scene_entity =
      m_scenes_root.lookup(std::string(scene_name).c_str());
...
flecs::query<> scenes_module_t::find_current_scenes() {
  flecs::entity current_state = find_current_state();
  if (current_state.is_valid()) {
    return m_world.query_builder<>(m_world.entity())
        .with(flecs::ChildOf, current_state)
        .with<spectre_scene_t>()
        .build();
  }
  return m_world.query_builder<>(m_world.entity())
      .with<spectre_scene_t>()
      .with(flecs::ChildOf, (flecs::entity_t)0)
      .build();
}
...
void scenes_module_t::push_state(flecs::entity state_prefab) {
  ...
  if (!m_state_stack.empty()) {
    ...
    if (current_state.is_valid()) {
      execute_recursive(
          current_state,
          [](ecs_world_t *world, ecs_entity_t entity_id, void *payload) {
            flecs::world flecs_world(world);
            ::spectre::modules::scripts::execute_on_exit(
                flecs_world.entity(entity_id));
          },
          nullptr);
  ...
  execute_recursive(
      ...
      [](ecs_world_t *world, ecs_entity_t entity_id, void *payload) {
        flecs::world flecs_world(world);
        ::spectre::modules::scripts::execute_on_enter(
            flecs_world.entity(entity_id));
      },
      nullptr);

  sandbox::modules::logs::info(const_cast<flecs::world &>(m_world),
                               "[Scenes Module] Pushed state '{}'.",
                               state_prefab.name().c_str());
}
...
void scenes_module_t::pop_state() {
  if (m_state_stack.empty()) {
    sandbox::modules::logs::warn(
        const_cast<flecs::world &>(m_world),
        "[Scenes Module] Attempted to pop state from an empty stack.");
    return;
  }
...
void scenes_module_t::execute_recursive(flecs::entity parent_entity,
                                        spectre_recursive_callback_t callback,
                                        void *payload) {
  if (!parent_entity.is_valid() || !callback)
    return;
  callback(m_world.c_ptr(), parent_entity.id(), payload);
  parent_entity.children([&](flecs::entity child_entity) {
    if (child_entity.name() !=
        "scripts") { // skip the scripts child container itself
...
flecs::entity scenes_module_t::create_state(std::string_view state_name) {
  ...
  if (!state_prefab.is_valid()) {
    sandbox::modules::logs::error(
        const_cast<flecs::world &>(m_world),
        "[Scenes Module] Cannot create state: prefab '{}' not found.",
        state_name);
    return flecs::entity::null();
  }
...
flecs::entity scenes_module_t::create_scene(std::string_view scene_name) {
  ...
  if (!scene_prefab.is_valid()) {
    sandbox::modules::logs::error(
        const_cast<flecs::world &>(m_world),
        "[Scenes Module] Cannot create scene: prefab '{}' not found.",
        scene_name);
    return flecs::entity::null();
  }
