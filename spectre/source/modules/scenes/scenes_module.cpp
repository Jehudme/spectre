...
void scenes_module_t::register_state(sandbox::properties properties) {
    ...
    state_prefab.children([&](flecs::entity child_entity) { child_entity.destruct(); });

    deserialize_state(state_prefab, std::move(properties));
    sandbox::modules::logs::info(const_cast<flecs::world&>(m_world), "[Scenes Module] Registered state '{}'.",
                                 state_name);
}
...
void scenes_module_t::register_scene(sandbox::properties properties) {
    ...
    flecs::entity scene_prefab = find_scene(scene_name);
    if (!scene_prefab.is_valid()) {
        scene_prefab = m_world.prefab(scene_name.c_str()).child_of(m_scenes_root).add<spectre_scene_t>();
    }
...
void scenes_module_t::push_state(flecs::entity state_prefab) {
    ...
    sandbox::modules::logs::info(const_cast<flecs::world&>(m_world), "[Scenes Module] Pushed state '{}'.",
...
void scenes_module_t::pop_state() {
    ...
    flecs::entity current_state = m_state_stack.top();
    if (current_state.is_valid()) {
        sandbox::modules::logs::info(const_cast<flecs::world&>(m_world), "[Scenes Module] Popping state '{}'.",
                                     current_state.name().c_str() ? current_state.name().c_str() : "unknown");
        execute_recursive(
            current_state,
            [](ecs_world_t* world, ecs_entity_t entity_id, void* payload) {
                flecs::world flecs_world(world);
...
void scenes_module_t::execute_recursive(flecs::entity parent_entity, spectre_recursive_callback_t callback,
                                        void* payload) {
...
flecs::entity scenes_module_t::create_state(std::string_view state_name) {
    ...
    if (!state_prefab.is_valid()) {
        sandbox::modules::logs::error(const_cast<flecs::world&>(m_world),
                                      "[Scenes Module] Cannot create state: prefab '{}' not found.", state_name);
        return flecs::entity::null();
    }
...
flecs::entity scenes_module_t::create_scene(std::string_view scene_name) {
    ...
    if (!scene_prefab.is_valid()) {
        sandbox::modules::logs::error(const_cast<flecs::world&>(m_world),
                                      "[Scenes Module] Cannot create scene: prefab '{}' not found.", scene_name);
        return flecs::entity::null();
    }
