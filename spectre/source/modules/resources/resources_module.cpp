...
static void
deserialize_resource_cb(ecs_world_t *world, ecs_entity_t entity,
                        sandbox_properties_handle_t properties_handle);
static sandbox_properties_handle_t
serialize_resource_cb(ecs_world_t *world, ecs_entity_t entity_id);

// Serializers for resource component
static sandbox_properties_handle_t
serialize_resource_comp_cb(ecs_world_t *world, ecs_entity_t entity) {
  if (!world || !entity)
    return {0};
  flecs::world flecs_world(world);
  const auto *comp =
      flecs_world.entity(entity).try_get<spectre_resource_component_t>();
  if (!comp || !comp->path)
    return {0};
...
static void deserialize_resource_comp_cb(ecs_world_t *world,
                                         ecs_entity_t entity,
...
resource_module_t::resource_module_t(flecs::world &world) : m_world(world) {
  ...
  spectre_serializer_component empty_serializer = {deserialize_empty,
                                                   serialize_empty};
  spectre_serializer_component resource_comp_serializer = {
      deserialize_resource_comp_cb, serialize_resource_comp_cb};

  register_resource_component(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_resource_component_t", &resource_comp_serializer);

  register_resource_loader_component(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_resource_loader_component_t", &empty_serializer);

  register_use_loader_relation(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_use_loader_relation_t", &empty_serializer);

  register_resource_flag(m_world.c_ptr());
  spectre::modules::serializer::register_serializer(
      m_world, "spectre_resource_flag_t", &empty_serializer);
  ...
  spectre::modules::serializer::register_serializer(m_world, "resources",
                                                    &resource_serializer);

  m_resources_serializer = m_world.entity(
      spectre::modules::serializer::find_serializer(m_world, "resources"));

  sandbox::modules::logs::info(const_cast<flecs::world &>(m_world),
                               "[Resources Module] Initialized successfully.");
}
...
deserialize_resource_cb(ecs_world_t *world, ecs_entity_t entity,
  ...
  if (module)
    module->deserialize_resource(flecs::world(world).entity(entity),
                                 sandbox::properties(properties_handle, false));
}

static sandbox_properties_handle_t
serialize_resource_cb(ecs_world_t *world, ecs_entity_t entity_id) {
  return spectre::modules::resources::serialize_resource(flecs::world(world),
                                                         entity_id);
}

void resource_module_t::register_resource_loader(std::string_view type,
                                                 ResourceLoader loader) {
  if (type.empty())
    return;
  flecs::entity loader_entity =
      m_loaders_root.lookup(std::string(type).c_str());
  if (!loader_entity.is_valid()) {
    loader_entity =
        m_world.entity(std::string(type).c_str()).child_of(m_loaders_root);
  }
  if (loader_entity.has<ResourceLoader>()) {
    sandbox::modules::logs::warn(
        m_world,
        "[Resources Module] Resource loader for type '{}' is being overridden.",
        type);
  }
  loader_entity.set<ResourceLoader>(loader);
}

void resource_module_t::register_resource(
  ...
  if (!m_resources_serializer.is_valid()) {
    sandbox::modules::logs::error(
        m_world, "[Resources Module] Resources serializer not available.");
    return;
  }
  ...
  for (const auto &key : keys) {
    ...
    spectre::modules::serializer::deserialize_entity(
        m_world, m_resources_serializer.id(), resource_entity.id(), handle);
  }
}

void resource_module_t::deserialize_resource(
    flecs::entity resource_entity, const sandbox::properties &properties) {
  if (!properties.is_valid() || !resource_entity.is_valid())
    return;
  std::string type;
  if (!properties.get<std::string>("type", type) &&
      !properties.get<std::string>("types", type)) {
    sandbox::modules::logs::error(
        m_world, "[Resources Module] Resource missing 'type'.");
    return;
  }
  std::string path;
  if (!properties.get<std::string>("path", path)) {
    sandbox::modules::logs::error(
        m_world, "[Resources Module] Resource missing 'path'.");
    return;
  }
  ...
  if (!loader_entity.is_valid()) {
    sandbox::modules::logs::error(
        m_world, "[Resources Module] Resource loader not found for type '{}'.",
        type);
    return;
  }
...
sandbox::properties
resource_module_t::serialize_resource(flecs::entity entity_to_serialize) {
...
bool resource_module_t::has_resource(std::string_view name) const {
  ...
  flecs::entity resource_entity =
      m_resources_root.lookup(std::string(name).c_str());
...
bool resource_module_t::is_resource(flecs::entity entity_to_check) const {
  return entity_to_check.is_valid() &&
         entity_to_check.has(flecs::IsA, m_resource_prefab);
}
...
bool resource_module_t::is_resource_loaded(
    flecs::entity resource_entity) const {
  if (!is_resource(resource_entity))
    return false;
...
void resource_module_t::load_resource(flecs::entity resource_entity) {
  ...
  flecs::entity loader_entity =
      resource_entity.target<spectre_use_loader_relation_t>();
  if (!loader_entity.is_valid()) {
    sandbox::modules::logs::error(
        m_world, "[Resources Module] Resource '{}' has no loader relation.",
  ...
  if (!loader || !loader->load_fn) {
    sandbox::modules::logs::error(
        m_world, "[Resources Module] Loader for resource '{}' is invalid.",
        resource_entity.name().c_str());
    return;
  }
