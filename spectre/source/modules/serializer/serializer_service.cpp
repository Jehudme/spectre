...
void spectre_serializer_register_serializer(ecs_world_t* entity_world, const char* type, const spectre_serializer_t* serializer) {
    if (!entity_world) return;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_serializer_service_t)) {
        if (service->api && service->api->register_serializer) {
            service->api->register_serializer(entity_world, type, serializer);
        }
    }
}

bool spectre_serializer_has_serializer(ecs_world_t* entity_world, const char* type) {
    if (!entity_world) return false;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_serializer_service_t)) {
        if (service->api && service->api->has_serializer) {
            return service->api->has_serializer(entity_world, type);
        }
    }
    return false;
}

ecs_entity_t spectre_serializer_find_serializer(ecs_world_t* entity_world, const char* type) {
    if (!entity_world) return 0;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_serializer_service_t)) {
        if (service->api && service->api->find_serializer) {
            return service->api->find_serializer(entity_world, type);
        }
    }
    return 0;
}

sandbox_properties_handle_t spectre_serializer_serialize_entity(ecs_world_t* entity_world, ecs_entity_t serializer, ecs_entity_t entity) {
    if (!entity_world) return {0};
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_serializer_service_t)) {
        if (service->api && service->api->serialize_entity) {
            return service->api->serialize_entity(entity_world, serializer, entity);
        }
    }
    return {0};
}

ecs_entity_t spectre_serializer_deserialize_entity(ecs_world_t* entity_world, ecs_entity_t serializer, sandbox_properties_handle_t props) {
    if (!entity_world) return 0;
    flecs::world flecs_world(entity_world);
    if (const auto* service = SANDBOX_GET_SERVICE(flecs_world, spectre_serializer_service_t)) {
        if (service->api && service->api->deserialize_entity) {
            return service->api->deserialize_entity(entity_world, serializer, props);
        }
    }
    return 0;
}
