...
sandbox_logs_api_t mock_logs_api = {[](ecs_world_t*, const char* msg) { std::cout << "[TRACE] " << msg << std::endl; },
...
TEST_CASE("Renderer Module: Rendering and Logging", "[renderer]") {
    ...
    SECTION("Entity with renderable component logs its rendering type") {
        ...
        flecs::entity rect_entity =
            world.entity("MyRectangle").add<spectre_renderable_t>().set<spectre_rectange_renderable_t>(rect_comp);
        ...
        flecs::entity poly_entity =
            world.entity("MyPolygon").add<spectre_renderable_t>().set<spectre_polygone_renderable_t>(poly_comp);
    ...
    SECTION("Renderables only draw when their state is active") {
        ...
        auto* scenes_module = const_cast<spectre::modules::scenes_module_t*>(
            &world.get_mut<spectre::modules::scenes_module_t>());
        ...
        flecs::entity scene1 =
            world.entity("Scene1").child_of(state1).add<spectre_scene_t>().set<spectre_state_use_scene_relation_t>({0});

        flecs::entity scene2 =
            world.entity("Scene2").child_of(state2).add<spectre_scene_t>().set<spectre_state_use_scene_relation_t>({0});
        ...
        flecs::entity r1 =
            world.entity("RectInState1").child_of(scene1).add<spectre_renderable_t>().set<spectre_rectange_renderable_t>(r);

        flecs::entity r2 =
            world.entity("RectInState2").child_of(scene2).add<spectre_renderable_t>().set<spectre_rectange_renderable_t>(r);
