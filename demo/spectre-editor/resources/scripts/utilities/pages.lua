...
function pages.find(page_type, page_name)
  if not pages.map[page_type] then return nil end
...
function pages.actions.switch_page(new_page, page_name)
  ...
  if pages.current_page == target_page then return end

  local switch_action = SwitchPageAction.new(pages.current_page, target_page)
  local action = Action.new(
    function() switch_action:redo() end,
    function() switch_action:undo() end,
...
function pages.run_test()
  ...
  page_menu.on_enter = function() log_trace("enter_menu") end
  page_menu.on_exit = function() log_trace("exit_menu") end

  local page_editor = Page.new()
  page_editor.on_enter = function() log_trace("enter_editor") end
  page_editor.on_exit = function() log_trace("exit_editor") end
  ...
  if pages.current_page == page_editor and #trace == 2 and trace[1] == "exit_menu" and trace[2] == "enter_editor" then
    ...
    sandbox.logs.error(world, "[Pages Test] Failed Switch Page Action. Current: " .. tostring(pages.current_page and pages.current_page.name or nil))
  end
