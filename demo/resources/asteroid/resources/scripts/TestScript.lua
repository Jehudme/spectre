...
function TestScript.on_update(self_id, scene_id, state_id)
  ...
  if PlayerStats and PlayerStats ~= 0 then
      ...
      if not world:has(self_id, PlayerStats) then
          ...
          sandbox.logs.info(world, "[TestScript] PlayerStats - Health: " .. tostring(health) .. ", Speed: " .. tostring(speed) .. ", Name: " .. tostring(name) .. ", Alive: " .. tostring(alive))
      ...
      sandbox.logs.warn(world, "[TestScript] Could not find dynamic component 'PlayerStats'! Type: " .. type(PlayerStats) .. " Value: " .. tostring(PlayerStats))
  end
