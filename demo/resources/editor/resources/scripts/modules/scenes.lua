...
local selected_mode = nil -- "state" or "scene"
...
local function save_item(mode, name, props)
    if props then
...
local function select_item(mode, name)
    selected_mode = mode
    selected_item = name
    ...
    if name then
        current_props = load_item(mode, name)
        selected_entity = nil
    end
end

function Scenes.on_enter()
    refresh_lists()
    selected_item = nil
    selected_entity = nil
    if current_props then
...
function Scenes.on_exit()
end

local function draw_hierarchy(props, path, name)
    local flags = bit.bor(64, 2048)
    if selected_entity == path then flags = bit.bor(flags, 1) end
    
    local children_path = path .. "/children"
    local children_keys = {}
    if props:has(children_path) then children_keys = props:keys(children_path) or {} end
    local has_children = (#children_keys > 0)
    
    if not has_children then flags = bit.bor(flags, 256, 16) end
    ...
    if imgui.BeginPopupContextItem("EntityCtx_" .. path) then
        ...
        if not is_root and imgui.MenuItem("Duplicate") then
            ...
            save_item(selected_mode, selected_item, props)
        end
    ...
    if open and not bit.band(flags, 256) ~= 0 then imgui.TreePop() end
end

function Scenes.on_update()
    ...
    imgui.Begin("Scenes Manager##Main", nil, window_flags)
    ...
    imgui.SameLine()
    if imgui.Button("Refresh") then refresh_lists() end
    ...
    if imgui.CollapsingHeader("States", bit.bor(32)) then
        for _, name in ipairs(states_list) do
            if search_str == "" or string.find(name:lower(), search_str:lower(), 1, true) then
                ...
                if imgui.Selectable(name, is_selected) then
                    if not is_selected then select_item("state", name) end
                end
                ...
                if imgui.BeginPopupContextItem("StateCtx_" .. name) then
                    ...
                    if imgui.MenuItem("Delete") then
                        sandbox.filesystem.remove_file(world, get_item_path("state", name))
                        if selected_mode == "state" and selected_item == name then select_item(nil, nil) end
                        refresh_lists()
                    end
                    imgui.EndPopup()
                end
    ...
    if imgui.CollapsingHeader("Scenes", bit.bor(32)) then
        for _, name in ipairs(scenes_list) do
            if search_str == "" or string.find(name:lower(), search_str:lower(), 1, true) then
                ...
                if imgui.Selectable(name, is_selected) then
                    if not is_selected then select_item("scene", name) end
                end
                ...
                if imgui.BeginPopupContextItem("SceneCtx_" .. name) then
                    ...
                    if imgui.MenuItem("Duplicate") then
                        ...
                        local props = load_item("scene", name)
                        local root_dump = props:sub("entities/" .. name):dump(0)
                        if root_dump then
                            local new_props = sandbox.Properties.new()
                            new_props:load(string.format('{"entities":{"%s": %s}}', new_name, root_dump), 0)
                            save_item("scene", new_name, new_props)
                            new_props:destroy()
                        else
                            sandbox.filesystem.copy(world, old_path, get_item_path("scene", new_name), false, true)
                        end
                        props:destroy()
                        refresh_lists()
                    end
                    if imgui.MenuItem("Delete") then
                        sandbox.filesystem.remove_file(world, get_item_path("scene", name))
                        if selected_mode == "scene" and selected_item == name then select_item(nil, nil) end
                        refresh_lists()
                    end
                    imgui.EndPopup()
                end
    ...
    if selected_item and current_props then
        ...
        for _, ent_name in ipairs(ent_keys) do
            draw_hierarchy(current_props, "entities/" .. ent_name, ent_name)
        end
        if #ent_keys == 0 then
            if imgui.Button("Add Root Entity") then
                ...
                if selected_mode == "state" then
                    current_props:set_string("entities/" .. selected_item .. "/components/State/dummy", "0")
                    current_props:clear("entities/" .. selected_item .. "/components/State/dummy")
                else
                ...
                save_item(selected_mode, selected_item, current_props)
            end
        end
    else
    ...
    if selected_entity and current_props then
        ...
        for _, comp_name in ipairs(comp_keys) do
            if imgui.CollapsingHeader(comp_name, bit.bor(32)) then
                ...
                if _G.modules["Prefabs"] and _G.modules["Prefabs"].Drawers[comp_name] then
                    if _G.modules["Prefabs"].Drawers[comp_name](current_props, selected_entity) then
                        modified = true
                    end
                else
                    local dyn_comps = _G.modules["Components"] and _G.modules["Components"].list_dynamic_components() or {}
                    local is_dyn = false
                    for _, d in ipairs(dyn_comps) do
                        if d == comp_name then is_dyn = true break end
                    end
                    if is_dyn then
                        if _G.modules["Components"].draw_dynamic_component(current_props, selected_entity, comp_name) then
                            modified = true
                        end
                    else
                ...
                -- State and Scene components cannot be removed from root entity
                local is_root_core = (selected_entity == "entities/" .. selected_item) and (comp_name == "State" or comp_name == "Scene")
        ...
        if imgui.Button("Add Component") then
            ...
            local excluded = {
                Scene = true, State = true, StateContext = true, SceneContext = true,
            ...
            if _G.modules["Prefabs"] then
                for k, _ in pairs(_G.modules["Prefabs"].Drawers) do
                    if not excluded[k] and not string.match(k, "Relation$") then
                        table.insert(available_components, k)
                    end
                end
            end
            if _G.modules["Components"] then
    ...
    if show_add_state_popup then imgui.OpenPopup("New State") end
    ...
    if show_add_scene_popup then imgui.OpenPopup("New Scene") end
    ...
    if show_rename_popup then imgui.OpenPopup("Rename Item") end
        ...
        if imgui.Button("Rename") then
            ...
            if new_name ~= "" and new_name ~= rename_target then
                ...
                if not sandbox.filesystem.exists(world, new_path) then
                    local props = load_item(selected_mode, rename_target)
                    local root_dump = props:sub("entities/" .. rename_target):dump(0)
                    if root_dump then
                        local new_props = sandbox.Properties.new()
                        new_props:load(string.format('{"entities":{"%s": %s}}', new_name, root_dump), 0)
                        save_item(selected_mode, new_name, new_props)
                        new_props:destroy()
                    ...
                    if selected_item == rename_target then
                        select_item(selected_mode, new_name)
                    end
                    refresh_lists()
                end
            end
    ...
    if imgui.BeginPopupModal("Add Assigned Prefab", nil, 64) then
        ...
        if sandbox.filesystem.exists(world, "project://scenes/prefabs") then
            ...
            for _, file in ipairs(files) do
                if string.sub(file, -5) == ".json" then
                    local name = string.match(file, "([^/\\]+)%.json$")
                    if name then table.insert(p_list, name) end
                end
            end
        end
        table.sort(p_list)
