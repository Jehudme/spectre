		...
		for _, comp_name in ipairs(comp_keys) do
			if imgui.CollapsingHeader(comp_name, bit.bor(32)) then
				...
				if _G.modules["Prefabs"] and _G.modules["Prefabs"].Drawers[comp_name] then
					...
					local dyn_comps = _G.modules["Components"] and _G.modules["Components"].list_dynamic_components()
						or {}
					local is_dyn = false
					...
					if is_dyn then
						if
							_G.modules["Components"].draw_dynamic_component(current_props, selected_entity, comp_name)
						then
							modified = true
						end
					else
